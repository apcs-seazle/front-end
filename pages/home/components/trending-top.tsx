...
function ListOut({items}) {
  return (
    <div id="all" className="grid grid-rows-5 grid-cols-2 grid-flow-col gap-y-px mx-40 auto-cols-max gap-x-20">
      {items.map((item, index) => (<div className="display:indivne-block px-2"><NFT id={item._id} nft={item} index={index}/></div>))}
    </div>
  )
}
    ...
    return (
        <div className="relative">
            <div className="flex flex-row space-x-10 ml-40 mt-20">
                <button className={`${isTrendingOrTop=='trending' ? "text-black" : "text-gray-400"} text-2xl font-semibold ${isTrendingOrTop=='trending' ? "underline" : ""} underline-offset-8`} onClick={()=>setTrendingOrTop('trending')}>
                    Trending
                </button>
                <button className={`${isTrendingOrTop=='top' ? "text-black" : "text-gray-400"} text-2xl font-semibold ${isTrendingOrTop=='top' ? "underline" : ""} underline-offset-8`}  onClick={()=>setTrendingOrTop('top')}>
                    Top
                </button>
            </div>
