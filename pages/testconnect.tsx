...
const connectmetamask = ()=>{
  ...
  useEffect(() => {
    ...
    const SellNFT = async ()=>{
      if(typeof window.ethereum != "undefined")
      {
          if(defaultAccount != "")
          {
              try{
                await contract()
                .methods.sellNFT(parseInt(selectId))
                .send({
                  from: defaultAccount
                })
    ...
    const testne = async ()=>{
      ...
      await axios.get("http://localhost:3030/idMinted/get")
        .then(res => {
          const ids = res.data;
          setIDMinted(ids);
          console.log((ids[0]))
    ...
    return (
        <div>
          ...
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={GetNFT}>
            Get all NFTs
          </button>
          <div>
          ...
          {selectId != "" &&
            <p>
              The NFT with ID: {selectId} is selected
            </p>
          }
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={SellNFT}>
            Sell NFT
          </button>

          <h2 className="text-2xl font-bold m-2">BUY NFT</h2>
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={buyNFT} >
            Buy NFT
          </button>

          <h2 className="text-2xl font-bold m-2">GET MONEY BACK</h2>
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={getMoney} >
            Get Money
          </button>
